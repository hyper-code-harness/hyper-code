...
### 11.6. Внешний ориентир: Procedural Graphs (Google)

> Yuxing Lu, Yicheng Chen, Shanchan Wu, Sercan Ö. Arık.
> **Procedural Graphs: Self-Evolving Execution Structures for LLM Agents.**
> Google / Georgia Tech / Peking University, arXiv:2609.09153 (сентябрь 2026).
> <https://arxiv.org/abs/2609.09153>

Самая близкая к §11 внешняя работа. Она реализует ровно наши два сцепленных цикла и даёт конкретику там, где у нас пока формулировки. Ниже — что стоит перенять.

#### Skill как граф переходов, а не как плоский список шагов

У нас `Skill.todoTemplate` — линейная последовательность, а `applicableWhen` / `contraindications` относятся к Skill целиком. В PG процедурное знание — направленный граф троек **(процедура, отношение, процедура)**: узлы абстрагируют вызовы инструментов, навыки и внутренние шаги рассуждения, рёбра задают допустимые переходы. Условия висят не на Skill, а **на ребре**:

```text
(cash_flow_forecast, LEADS_TO, fund_raising_request)
  condition: прогнозируемый runway ниже защитного буфера
  guidance:  подавай заявку заранее — финансирование приходит с задержкой
  pitfalls:  не подавай вторую заявку, пока первая в работе
```

Это снимает нашу проблему «слишком общий Skill работает по-разному в разных контекстах» без немедленного `split`: разные ветки живут в одном графе как разные переходы. Отношений достаточно четырёх: `LEADS_TO`, `TRIGGERS`, `PROVIDES_INPUT_FOR`, `CONVERGES_TO`. Графы получаются маленькими — 7–17 узлов и 7–27 троек на задачу.

#### Локализация: подсказка из окрестности, а не весь Plan в промпт

У нас Plan отдаётся исполнителю целиком, и понятия «где я сейчас внутри Plan» в модели нет. В PG на каждом шаге работают три операции:

1. **locate** — последнее действие агента (например, имя вызванного инструмента) точным матчингом сопоставляется с узлом графа; не совпало — берётся весь граф;
2. **extract** — извлекается связная окрестность узла на `h=2` перехода вперёд плюс окно последних `w=3` шагов траектории (важна именно связность: `submit` без предшествующего `check_answer` теряет смысл, top-k similarity этого не даёт);
3. **generate** — отдельная guidance-модель превращает подграф в короткую ситуативную подсказку и дописывает её в промпт исполнителя.

Подсказка **смещает** выбор следующего действия, но не диктует его — исполнитель сохраняет свободу рассуждения. Побочный эффект, полезный нам по причинам контекста: локализованная подсказка сокращает число шагов агента относительно варианта «весь граф в промпт».

#### Validation gate: измеримое условие вместо «достаточных оснований»

Наш §11.4 говорит «нужен повторяющийся паттерн, причинная связь, внешний тест» — верно, но неоперационально. PG превращает это в цикл из четырёх шагов, выполняемый **оффлайн, после батча задач**:

1. **Diagnostic rollout** — прогон батча на текущем графе, траектории делятся на успешные и неуспешные;
2. **Feedback-driven mutation** — LLM-рефайнер сравнивает удачные трассы с провальными, ищет повторяющиеся циклы-ошибки и полезные сокращения, выдаёт набор правок `Add` / `Delete` (правка атрибутов = удалить ребро и добавить заново); правки применяются к **копии** графа;
3. **Validation gating** — структурно невалидные кандидаты отбрасываются до прогона; остальные измеряются на **независимом валидационном наборе**, и кандидат принимается только если `S_val(новый) ≥ S_val(текущего)`;
4. **Rejection memory** — см. ниже.

Это даёт нам операциональный ответ на открытый вопрос 11 («какой объём доказательств достаточен для автоматического изменения Skill»): достаточен тот, который не ухудшает измеренный результат на наборе, независимом от батча, породившего правку.

#### Rejection memory: где хранить неуспех

Прямой ответ на наш открытый вопрос 5. Отклонённый кандидат не выбрасывается: граф, предложенные правки, породившие их траектории и результаты валидации пишутся в `H_rejected` и подаются рефайнеру на следующем раунде как **негативное свидетельство при предложении правок**.

Ключевая деталь, которая и снимает риск чрезмерного избегания: отклонённый кандидат **никогда не становится стартовым графом следующего раунда** и **не превращается в запрет для исполнителя**. Избегание живёт в контуре памяти (влияет на то, что рефайнер предлагает), а не в контуре исполнения (не сужает пространство действий агента). Это ровно то разделение, которого нам не хватало: сейчас в `reflection.md` отклонённые `candidate_rules` просто теряются, поэтому один и тот же неудачный вывод может предлагаться бесконечно.

#### Что показывают их измерения

- PG первый или разделяет первое место в 21 из 24 пар «модель × бенчмарк» (HotpotQA, MultiChallenge, GDPval, ALFWorld, τ-bench, BFCL v3, EnterpriseArena); лучшие приросты +9.0 / +7.4 / +7.0 пункта.
- Граф можно вырастить **с нуля**: стартуя с `START → END`, цикл догоняет рукописный экспертный граф.
- Испорченный экспертный приор чинится: с ~59% до 92.86%.
- Веса модели не обучаются — учится только граф. Для нас это важно: вся процедурная память остаётся данными, которые можно посмотреть, отредактировать руками и откатить.
- Показателен трек самоэволюции CFO-агента: раунд 1 находит «хребет» (`проверить деньги → прогноз runway → сохранить заметку → проверить рынок → решение о капитале`), раунд 2 вставляет `recall_notes` и вместе с `save_note` даёт внешнюю рабочую память между месяцами (вызовов инструментов в месяц: 17.23 → 3.08), раунд 7 **удаляет** узел «ничего не делать», уводивший в провал. То есть `Delete` не менее важен, чем `Add`.

#### Что из этого предлагается взять

| Идея PG | Как ложится на нашу модель |
|---|---|
| Условия на рёбрах (`condition` / `guidance` / `pitfalls`) | обогатить `Skill.todoTemplate`: переход между шагами получает свои условия вместо общего `applicableWhen` |
| `locate` активного узла | ввести «текущую позицию» в Plan — сейчас её нет ни в `session.plan`, ни в модели §2.4 |
| Окрестность `h=2` вместо полного Plan в промпте | часть работы по управлению контекстом, смыкается с `continuous-context-compaction.md` |
| Validation gate `S_val(new) ≥ S_val(old)` | операциональная замена «достаточных оснований» в §11.4; требует набора повторяемых задач-проверок |
| `H_rejected` | недостающий элемент §11.3: к `add / rate / edit / split / merge / deprecate` добавляется `reject-and-remember` |
| Оффлайн-раунды по батчу, а не после каждого действия | совпадает с нашими «естественными точками» консолидации из §11.5 |

Главное отличие их постановки от нашей: PG оптимизирует **способ исполнения** при фиксированной цели и не моделирует intent, goal, participation и control stream. Наши §§2–10 остаются нашей задачей; §11 — та часть, где у нас теперь есть проверенный образец.

---


### 11.7. Обратный цикл: обучение на живой работе

§11.6 описывает оффлайн-тренировку: нужны набор повторяемых задач, автоматическая оценка и возможность прогнать сценарий заново. У нас нет ни одного из трёх условий: задачи уникальны, эталонного ответа нет, месяц работы заново не прогонишь. Значит, цикл надо перевернуть.

#### Два переворота

**Валидация назад, а не вперёд.** У PG дорого именно то, что кандидата надо *действительно прогнать* на новых задачах. Но у нас есть то, чего нет у них — архив настоящих эпизодов в Postgres. Кандидатную правку можно проверять **реплеем по прошлому**: взять 30–40 записанных точек принятия решения, где агент реально ошибался, и спросить: с этой подсказкой в промпте выбор был бы другим? Действовать в мире не нужно — нужен один дешёвый вызов модели на точку. Это тот же validation gate, только набор не придуман заранее, а накоплен работой.

**Гейт после, а не до.** Вместо «докажи, что лучше, потом входи» — правка входит сразу со статусом `trial` и начинает считать свои исходы в реальной работе. N применений без трения и с зелёными проверками → `active`; первое явное опровержение → в `H_rejected`. Тогда поток обычных задач *сам* становится валидационным набором, а обучение идёт непрерывно.

Награда берётся из двух бесплатных источников: **машинного** (typecheck прошёл, `runtime.docs.validate` зелёный, тесты, миграция накатилась) и **пользовательского** (детектор трения из `reflection.md`: поправка — минус, молчание и переход к следующей задаче — слабый плюс).

Начинать стоит с узкого класса задач, где проверка бесплатна и автоматична: «добавить runtime-функцию», «завести плагин», «написать миграцию», «починить упавший тест». Таких эпизодов в истории уже сотни.

#### Привязка к Goal

У PG граф один на бенчмарк и задача фиксирована, поэтому вопрос «к чему привязана процедура» не возникает. У нас возникает и решается через `goalPattern` из §11.2:

```text
Goal → recall кандидатов по типу цели и контексту
     → выбранный Skill со своим графом
     → локализация «где я сейчас» внутри этого графа
```

Их граф — это содержимое **одного** Skill, а не вся процедурная память. Траектории копятся как эпизоды с привязкой к `goalId`, и правки графа предлагаются только по эпизодам **той же семьи целей** — иначе опыт «добавить runtime-функцию» начнёт править процедуру «разобрать почту».

#### Узел — это функция, а не имя шага

Здесь наша позиция сильнее исходной. У PG узлы — абстрактные имена шагов, а знание живёт текстом на рёбрах и попадает в промпт как совет. У нас узел — настоящая `ctx.fns.<mod>.<fn>` с JSON Schema параметров, `returnType` и typecheck. Следствия:

1. **Часть рёбер не надо выучивать.** `PROVIDES_INPUT_FOR` выводится из типов, и компилятор проверяет это лучше любого рефайнера.
2. **Траектория уже записана и разбираема.** Это `role='tool'` сообщения с `tool_call_id` в Postgres — готовая обучающая выборка, а не то, что надо специально инструментировать.
3. **Устойчивый подграф можно свернуть в узел.** `procs.dev.createFunction` — и десять шагов становятся одним вызовом с осмысленным именем.

Третий пункт объясняет, почему их графы застревают на 7–17 узлах: без механизма абстракции граф либо мелкий, либо неуправляемый. У нас абстракция рекурсивная, а побочный эффект — сжатие контекста: вместо пятнадцати tool-вызовов в транскрипте один, и компактить нечего. Версионирование и откат даёт git.

#### Лестница зрелости процедуры

```text
guidance   — текст на ребре; правится дёшево, ошибиться не страшно
   ↓
 trial    — применяется и считает исходы на реальном потоке задач
   ↓
compiled  — после N успехов на разных задачах подграф застывает в функцию
```

Перепрыгивать ступени нельзя. Компиляция фиксирует процедуру жёстко и убивает ту самую мягкость «подсказка смещает, но не диктует», ради которой в PG всё и работает. Обратный ход — разжать функцию назад в рёбра — должен остаться возможным.

Оговорка про шум: при онлайн-обучении каждое измерение — один шумный эпизод, поэтому порог промоушена должен считать *повторение на разных задачах*, а не одну удачу. Активный Skill не правится напрямую никогда — только через `trial`-рёбра (продолжение §11.4).
...
5. Как хранить неуспех Skill, не создавая чрезмерного избегания? — *частичный ответ в §11.6: rejection memory влияет только на предложение правок, но не на пространство действий исполнителя.*
...
11. Какой объём доказательств достаточен для автоматического изменения Skill? — *частичный ответ в §11.6: прогон кандидата на независимом валидационном наборе с правилом `S_val(new) ≥ S_val(old)`; открытым остаётся, где нам взять такой набор для пользовательских задач.*
...
- Yuxing Lu, Yicheng Chen, Shanchan Wu, Sercan Ö. Arık. **Procedural Graphs: Self-Evolving Execution Structures for LLM Agents.** Google / Georgia Tech / Peking University, arXiv:2609.09153, сентябрь 2026. <https://arxiv.org/abs/2609.09153> — разобрано в §11.6.
- `docs/reflection.md` — детектор трения и буфер `candidate_rules`; естественное место для `H_rejected`.
- `docs/workflow-engine-design.md` — durable-исполнение задач и ожиданий.
- `docs/continuous-context-compaction.md` — управление контекстом, смыкается с локализованной подсказкой.

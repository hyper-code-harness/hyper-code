...
4. **`docs`** — ✅ портирован 7 сентября как user-плагин `~/.hyper/user/docs` (схема `docs`
   в hyper, cron `docs-sync` каждый час, UI `/docs`). Данные не копировались: индекс пересобран
   из файлов `~/docs` (эмбеддинги теперь через `embeddings.embed`, 1536d вместо 3072d).
   Осталось выключить `docs-sync` в uniskill. **`docset`** — следующий: ~47k секций, та же
   логика чанков; решить, не заменяет ли его hyper-овский `functions`/RAG.

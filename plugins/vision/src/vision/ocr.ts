 ...
 * - qwen: Qwen3-VL-8B via LM Studio (see vision.qwenOcr). Most accurate engine tested, clean Markdown tables, preserves symbols, but ~5-30 s per image (grows with text volume), no boxes/confidence, needs LM Studio with the model downloaded (server and model are started on demand by vision.ensureModel; first call +5-15 s, ~10 GB RAM, auto-unload after 1 h idle), and generative output can hallucinate or loop — check truncated.

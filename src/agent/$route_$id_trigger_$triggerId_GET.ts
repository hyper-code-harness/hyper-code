...
export default async function (ctx: Context, _session: Session | null, opts: { req: Request; params: Record<string, string> }) {
  ...
  const rows = runs.map((run: any) => `<article class="rounded-lg border border-ui-border bg-base-100/45 p-3"><div class="flex items-center gap-2"><span class="badge badge-sm">${esc(run.outcome)}</span><span class="text-[10px] text-base-content/45">${esc(fmt(run.createdAt))}</span><span class="ml-auto font-mono text-[9px] text-base-content/35">#${esc(run.id)}</span></div>${run.error ? `<pre class="mt-2 text-error">${esc(run.error)}</pre>` : ""}${run.result != null ? `<pre class="mt-2">${esc(json(run.result))}</pre>` : ""}<div class="mt-2 text-[9px] text-base-content/35">scheduled ${esc(fmt(run.scheduledAt))}</div></article>`).join("");
  const html = `<section data-popup-content data-popup-title="Trigger audit" data-popup-kind="trigger-audit" class="space-y-4">${stats}<div><div class="text-[10px] font-semibold uppercase tracking-wide text-base-content/40">${esc(trigger.kind)} · ${esc(trigger.status)}</div><div class="mt-1 text-sm text-base-content/80">${esc(trigger.prompt)}</div><div class="mt-1 font-mono text-[9px] text-base-content/35">${esc(trigger.id)}</div></div>${trigger.lastError ? `<div class="rounded-lg bg-error/10 p-3 text-error">${esc(trigger.lastError)}</div>` : ""}<div><h3 class="mb-2 text-xs font-semibold">Run history</h3><div class="space-y-2">${rows || '<div class="text-base-content/40">No runs yet.</div>'}</div></div></section>`;

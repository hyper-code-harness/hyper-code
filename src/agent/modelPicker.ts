    ...
    const providerPanels = entries.map((entry, providerIndex) => {
        ...
        const modelPanels = entry.accounts.map((account, accountIndex) => {
            ...
            const rows = entry.models.map(baseModel => {
                ...
                return `<form hx-post="/agent/${encodeURIComponent(id)}/model" hx-swap="none" class="contents"><input type="hidden" name="model" value="${esc(target)}"><input type="hidden" name="scope" value="agent">${ctx.fns.procs.ui.button({ action: "select-model", html, type: "submit", disabled: selected || account.needsReconnect, class: `flex w-full items-center gap-2 rounded-lg text-left ${selected ? "border-primary/30 bg-primary/10 text-primary" : "text-base-content/70"}`, attrs: selected ? { "aria-current": "true" } : account.needsReconnect ? { title: "Reconnect this account in LLM connections before selecting it" } : {} })}</form>`;
            }).join("");

...
export default function (ctx: Context, _session: Session | null, opts: {
    ...
    if (opts.section === "automation") {
        ...
        const rows = triggers.map((trigger: any) => {
            ...
            const next = trigger.nextAt ? new Date(Number(trigger.nextAt)).toLocaleString() : trigger.status;
            return `<li class="rounded-lg border border-ui-border bg-base-100/35 px-2.5 py-2"><div class="flex items-center gap-2"><span class="badge badge-sm">${esc(kindLabel[trigger.kind] ?? trigger.kind)}</span><span class="truncate text-[10px] text-base-content/45">${esc(detail)}</span></div><div class="mt-1 line-clamp-3 text-xs leading-5 text-base-content/75">${esc(trigger.prompt)}</div><div class="mt-1 text-[9px] text-base-content/35">next ${esc(next)}</div><div class="mt-2 flex gap-1"><form hx-post="/agent/${id}/triggers" hx-swap="none"><input type="hidden" name="triggerId" value="${esc(trigger.id)}"><button class="btn btn-xs" name="action" value="run">Run now</button></form><form hx-post="/agent/${id}/triggers" hx-swap="none"><input type="hidden" name="triggerId" value="${esc(trigger.id)}"><button class="btn btn-xs text-error" name="action" value="cancel">Cancel</button></form></div></li>`;
        }).join("");
        ...
        const add = `<div class="mt-3 space-y-2"><details><summary class="cursor-pointer text-[11px] font-medium">Add wake-up</summary><div class="pt-2">${wakeForm}</div></details><details><summary class="cursor-pointer text-[11px] font-medium">Add schedule</summary><div class="pt-2">${cronForm}</div></details><details><summary class="cursor-pointer text-[11px] font-medium">Add watch</summary><div class="pt-2">${watchForm}</div></details></div>`;
        const cancelAll = triggers.length ? `<form hx-post="/agent/${id}/triggers" hx-swap="none" hx-confirm="Cancel all triggers?" class="mt-3"><button class="btn btn-xs text-error" name="action" value="cancelAll">Cancel all</button></form>` : "";
        const parkedHtml = ctx.fns.ui.parkedCard?.({ agent, models: opts.models, accounts: opts.accounts }) ?? "";
        return inspectorSection({ title: "Automation", icon: "lightning", badge: triggers.length ? String(triggers.length) : statusBadge({ label: "off" }), html: `${parkedHtml}${parkedHtml ? '<div class="mt-2"></div>' : ''}${rows ? `<ul class="space-y-2">${rows}</ul>` : '<div class="text-[11px] text-base-content/40">No active triggers.</div>'}${cancelAll}${add}`, collapsible: true, open: !!parkedHtml });
    }

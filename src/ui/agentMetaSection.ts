...
export default function (ctx: Context, _session: Session | null, opts: {
    ...
    if (opts.section === "automation") {
        ...
        const renderTrigger = (trigger: any, active: boolean) => {
            ...
            const inspection = runError ? `<div class="mt-1 break-words rounded bg-error/10 px-2 py-1 text-[10px] text-error">${esc(runError)}</div>` : result && result !== "null" ? `<div class="mt-1 line-clamp-3 break-words rounded bg-base-200/60 px-2 py-1 font-mono text-[9px] text-base-content/55">${esc(result)}</div>` : "";
            ...
            return `<li class="rounded-lg border border-ui-border bg-base-100/35 px-2.5 py-2"><div class="flex items-center gap-2"><span class="badge badge-sm">${esc(kindLabel[trigger.kind] ?? trigger.kind)}</span><span class="truncate text-[10px] text-base-content/45">${esc(detail)}</span><span class="ml-auto text-[9px] text-base-content/35">${esc(trigger.status)}</span></div><div class="mt-1 line-clamp-3 text-xs leading-5 text-base-content/75">${esc(trigger.prompt)}</div><div class="mt-1 text-[9px] text-base-content/35">${esc(when)} · ${esc(runState)}</div>${inspection}${controls}</li>`;
        };
        ...
        const wakeForm = `<form hx-post="/agent/${id}/triggers" hx-swap="none" class="space-y-2"><input type="hidden" name="action" value="wake"><input name="prompt" value="Continue scheduled work" required class="input input-bordered input-sm w-full text-xs"><div class="flex items-center gap-2"><input name="minutes" type="number" min="1" max="10080" value="5" class="input input-bordered input-sm w-20 text-xs"><span class="text-[10px] text-base-content/40">minutes</span><button class="btn btn-xs btn-primary ml-auto">+ Wake-up</button></div></form>`;
        const cronForm = `<form hx-post="/agent/${id}/triggers" hx-swap="none" class="space-y-2"><input type="hidden" name="action" value="cron"><input name="prompt" placeholder="Prompt" required class="input input-bordered input-sm w-full text-xs"><div class="grid grid-cols-2 gap-2"><input name="expression" value="0 9 * * *" required class="input input-bordered input-sm text-xs"><input name="timezone" value="UTC" required class="input input-bordered input-sm text-xs"></div><button class="btn btn-xs btn-primary">+ Schedule</button></form>`;
        const watchForm = `<form hx-post="/agent/${id}/triggers" hx-swap="none" class="space-y-2"><input type="hidden" name="action" value="watch"><input name="prompt" placeholder="Prompt when ready" required class="input input-bordered input-sm w-full text-xs"><div class="grid grid-cols-2 gap-2"><select name="predicate" class="select select-bordered select-sm text-xs"><option value="file.exists">File exists</option><option value="http.ok">HTTP OK</option><option value="db.rows">DB rows</option><option value="runtime.fn">Runtime function</option></select><input name="value" placeholder="path / URL / SQL / function" required class="input input-bordered input-sm text-xs"></div><div class="flex gap-2"><input name="everySeconds" type="number" min="5" value="60" title="Check every seconds" class="input input-bordered input-sm w-20 text-xs"><input name="timeoutMinutes" type="number" min="0" value="0" title="Timeout minutes; 0 means none" class="input input-bordered input-sm w-20 text-xs"><select name="mode" class="select select-bordered select-sm text-xs"><option value="once">Once</option><option value="edge">Every edge</option></select><button class="btn btn-xs btn-primary ml-auto">+ Watch</button></div></form>`;
        const addPopupId = `agent-trigger-add-${String(agent.id).replace(/[^a-zA-Z0-9_-]/g, "-")}`;
        const addButton = ctx.fns.procs.ui.button({ label: "Add trigger", html: '<i class="ph ph-plus" aria-hidden="true"></i><span>Add trigger</span><i class="ph ph-caret-down text-[10px]" aria-hidden="true"></i>', tone: "primary", size: "xs", action: "add-trigger", class: "mt-3 w-full justify-center gap-1.5", attrs: { popovertarget: addPopupId, "aria-haspopup": "dialog", style: `anchor-name:--${addPopupId}` } });
        const add = `${addButton}<div id="${esc(addPopupId)}" popover style="position-anchor:--${esc(addPopupId)}" class="inplace-popup-panel w-72 max-w-[calc(100vw-2rem)]"><div class="mb-2 text-[10px] font-semibold uppercase tracking-wide text-base-content/40">New trigger</div><div class="space-y-2"><details class="rounded-lg border border-ui-border"><summary class="flex cursor-pointer list-none items-center gap-2 px-3 py-2 text-xs font-medium"><i class="ph ph-alarm"></i><span>Wake-up</span><span class="ml-auto text-[10px] text-base-content/40">once</span></summary><div class="border-t border-ui-border p-3">${wakeForm}</div></details><details class="rounded-lg border border-ui-border"><summary class="flex cursor-pointer list-none items-center gap-2 px-3 py-2 text-xs font-medium"><i class="ph ph-calendar-clock"></i><span>Schedule</span><span class="ml-auto text-[10px] text-base-content/40">cron</span></summary><div class="border-t border-ui-border p-3">${cronForm}</div></details><details class="rounded-lg border border-ui-border"><summary class="flex cursor-pointer list-none items-center gap-2 px-3 py-2 text-xs font-medium"><i class="ph ph-eye"></i><span>Watch condition</span><span class="ml-auto text-[10px] text-base-content/40">event</span></summary><div class="border-t border-ui-border p-3">${watchForm}</div></details></div></div>`;
        const cancelAll = activeTriggers.length ? `<form hx-post="/agent/${id}/triggers" hx-swap="none" hx-confirm="Cancel all triggers?" class="mt-3"><button class="btn btn-xs text-error" name="action" value="cancelAll">Cancel all</button></form>` : "";
        const parkedHtml = ctx.fns.ui.parkedCard?.({ agent, models: opts.models, accounts: opts.accounts }) ?? "";
        return inspectorSection({ title: "Automation", icon: "lightning", badge: activeTriggers.length ? String(activeTriggers.length) : statusBadge({ label: "off" }), html: `${parkedHtml}${parkedHtml ? '<div class="mt-2"></div>' : ''}${rows ? `<ul class="space-y-2">${rows}</ul>` : '<div class="text-[11px] text-base-content/40">No active triggers.</div>'}${cancelAll}${add}${history ? `<details class="mt-3"><summary class="cursor-pointer text-[11px] font-medium">Recent runs</summary><ul class="mt-2 space-y-2">${history}</ul></details>` : ""}`, collapsible: true, open: !!parkedHtml });
    }

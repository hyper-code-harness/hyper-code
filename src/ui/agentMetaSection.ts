...
export default function (ctx: Context, _session: Session | null, opts: {
    ...
    if (opts.section === "automation") {
        ...
        const wakeForm = `<form hx-post="/agent/${id}/triggers" hx-swap="none" class="space-y-2"><input type="hidden" name="action" value="wake"><input name="prompt" value="Continue scheduled work" required class="input input-bordered input-sm w-full text-xs"><div class="flex items-center gap-2"><input name="minutes" type="number" min="1" max="10080" value="5" class="input input-bordered input-sm w-20 text-xs"><span class="text-3xs text-faint">minutes</span><button class="btn btn-xs btn-primary ml-auto">+ Wake-up</button></div></form>`;
